...
async def send_add_command(message: types.Message):
    ...
    args = message.get_args()
    if message.chat.id != 1020239695:
        return await bot.send_message(message.chat.id, "вы не можете добавлять рыбов(")
     if not args:
         return await bot.send_message(message.chat.id, f"Сейчас есть {len(ribov_list)}рыбов в наличии.Сколько рыбов вы хотите?")
     else:
         if args.isdigit():
             args = int(args)
            if args > len(ribiv_list):
                return await bot.send_message(message.chat.id, f"Мы сожалеем, но рыбов не хватит, попробуйте ввести число поменьше")
        else:
            for i in range(args):
              # print(i
              ribov_list.pop()

            return await bot.send_message(message.chat.id, f"Ошибка! Попробуйте ещё раз!")
